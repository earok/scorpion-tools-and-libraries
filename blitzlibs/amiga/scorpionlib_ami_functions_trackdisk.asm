;Trackdisk code from PHX (Solid Gold, Trap Runner etc)
...
MFM_GAPSIZE	equ	$200		; gap in words at track-start
MFM_WRITELEN	equ	$1760+MFM_GAPSIZE+1	; one extra word for hw bug
...
;Everything after this should be from the trackdisk file itself
td_init:
; Initialize the trackloader and check for connected drives.
	...
	; Motor off for all drives, deselect all.
	bsr	motor_off_all
...
.exit:
	...
	ifd	DIAGNOSTIC
	beq	.ok
	; invoke the debugger on read errors
	PANICW	trackdisk,d6
.ok:
	endif
...
.2:	move.l	(a2)+,d7		; data longword to encode into MFM
	...
	; we will write one extra word as the last three bits may be lost
	bsr	write_nulls
...
;---------------------------------------------------------------------------
write_gap:
; Write a gap with 0-bytes (MFM-encoded as $aaaa words).
; d0 = number of $aaaa-words / 2 - 1 to write
; a0 = MFM destination pointer
; d3 = MFM data mask
; -> a0 = new MFM pointer
...
.1:	moveq	#78,d0
	...
	; became low (happens for PC-drives or Amiga Technologies drives).
	st	MotorOn
	move.l	a0,d2
